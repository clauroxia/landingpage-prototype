...
class Subscriber < ApplicationRecord
  def has_one_preference
    errors.add(:preferences, I18n.t("errors.noPreferences")) if preferences.empty?
  end
  ...
  def quality_score
    ...
    errors.add(:email, I18n.t("errors.noValidEmail")) if response["quality_score"].to_f < 0.7
  end
  # Association
  ...
  validates :email,
            format: { with: /\A[\w+\-.]+@[a-z\d\-]+(\.[a-z\d\-]+)*\.[a-z]{2,3}\z/,
                      message: I18n.t("errors.invalidFormatEmail") }, unless: proc { |a|
                                                                                a.email.blank?
                                                                              }
